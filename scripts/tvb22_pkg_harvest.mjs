// TVB-22 package-port parity harvest -- per symbol: deep-load the ACTIVE
// strategy chart to the data floor, then for EACH pre-registered arm
// (A1/A2/A3) set the package strategy's Arm input in place, wait for the
// Strategy Tester report to settle, and dump the FULL reportData().trades
// list of the TFC-MT PACKAGE [TVB-22] strategy (matched by TITLE, so a
// co-mounted CONTROL never aliases in).
// Output: analysis/reference/pkg_parity/tvb22_{coin}_{arm}_trades.json
// Field mapping vs the TVB-20 harvest: package entries CARRY A COMMENT
// (pattern zipcode + trigger, e.g. "2-2u t=73.711"), which displaces the
// order id in trades[].e.c -- direction therefore comes from e.tp
// ("le"/"se" = long/short entry; probed live 2026-08-09).
// PRECONDITIONS: TFC-MT PACKAGE [TVB-22] is mounted on the active chart of
// the TVB18-parity layout and the Strategy Tester panel is open.
// Usage: node scripts/tvb22_pkg_harvest.mjs [COIN ...] (default GOOGL TSLA DRAM)
...
try {
  ...
  for (const coin of COINS) {
    ...
    for (const arm of ARMS) {
      ...
      const path = `${OUT_DIR}/tvb22_${coin}_${arm.tag}_trades.json`;
